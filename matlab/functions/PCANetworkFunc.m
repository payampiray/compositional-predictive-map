...
%% Place cell properties
 ...
 %In order array positions
    cellPositions_X = (mod(0:(Struct.Nx-1),Struct.Nx)+.2)*Struct.maxx/Struct.Nx;
    cellPositions_Y = (mod(0:(Struct.Ny-1),Struct.Ny)+.2)*Struct.maxy/Struct.Ny;
...
elseif strcmp(Struct.arragmentPC,'rand')
    centers = [reshape(X,1,[]); reshape(Y,1,[])];
    dx = diff(cellPositions_X);
    dx = dx(1);
    placeCenters = (rand(size(centers))-0.5)*dx + centers;

elseif strcmp(Struct.arragmentPC,'priori')
    placeCenters = Struct.centers;    
...
[~, cur_s] = min(sum((placeCenters - [x; y]).^2));
...
t=0;
J_history = {};
if ~isempty(Struct.step_cost)
    D = zeros(Struct.N1, Struct.N1);
end
...
while t<Struct.simdur
    ...
    if ~isempty(Struct.step_cost)
        ...
        [~, next_s] = min(dist);
        Struct.alpha = .1;
%         one = zeros(1, length(squareDists));
%         one(cur_s) = 1;
%         D(cur_s, :) = D(cur_s, :) + Struct.alpha*(one + exp(-Struct.step_cost)*D(next_s, :) - D(cur_s, :) );
% 
%         one = zeros(1, length(squareDists));
%         one(next_s) = 1;        
%         D(next_s, :) = D(next_s, :) + Struct.alpha*(one + exp(-Struct.step_cost)*D(cur_s, :) - D(next_s, :) );

        one = zeros(2, length(squareDists));
        cur_states = [cur_s; next_s];
        next_states = [next_s; cur_s];
        one(1, cur_states(1)) = 1;
        one(2, cur_states(2)) = 1;
        D(cur_states, :) = D(cur_states, :) + Struct.alpha*(one + exp(-Struct.step_cost)*D(next_states, :) - D(cur_states, :) );
    ...
    if paint
        ...
        n_mat=mulMatCross(zeros(Struct.Nx),zeros(Struct.Ny));
...
Struct.J = J';    
Struct.J_history = {J_history};

if ~isempty(Struct.step_cost)
    Struct.D = exp(-Struct.step_cost)*D;
end
